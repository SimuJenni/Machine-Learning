...
if isOctave
    ...
    VLFEAT_FOLDER = '~/src/vlfeat/vlfeat-0.9.17'; %Put here the path to the root floder of VlFeat
...
for i=0:115     
    ...
    %normalization
    [NormFaces] = normalizeData_SIMONJENNI(Faces);
    ...
    k = i*5+1;
    %k=300;
    Efaces = pca_SIMONJENNI(NormFaces,k);

    % Project Data to PCA basis
    DataTrain1 = project_SIMONJENNI(DataTrain, Efaces);
    DataITest1 = project_SIMONJENNI(DataITest, Efaces);

    for j=1:10

        % SVM
        [w, b] = vl_svmtrain(DataTrain1', y, lambda, 'MaxNumIterations', svm_iters);


        %%
        % Test Svm.
        % compute the test scores of the SVM
        scores = (DataITest1*w + b)';% complete the code
        classifierOutput = (scores >= 0.0) - (scores < 0.0);
        % accuracy
        good = classifierOutput == LabelsTest;
        %accuracy = 100*sum(good)/size(good,2);
        Acc(i+1,j) = 100*sum(good)/size(good,2);
        fprintf( 'accuracy: %f%% (%d/%d)\n', Acc(i+1,j), sum(good), size(good,2) );
    end
end
Avg= sum(Acc,2)/10;